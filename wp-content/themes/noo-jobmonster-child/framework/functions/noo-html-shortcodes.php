...
if(!function_exists('noo_recent_news_shortcode')){
	function noo_recent_news_shortcode($atts, $content = null){
		...
		$visibility       = ( $visibility      != ''     ) && ( $visibility != 'all' ) ? esc_attr( $visibility ) : '';
		$class            = ( $class           != ''     ) ? 'noo-recent-news posts-loop grid' . esc_attr( $class ) : 'noo-recent-news posts-loop grid';
		$class           .= noo_visibility_class( $visibility );
						...
						<div class="loop-item-wrap">
						    <div class="loop-item-featured">
						        <a href="<?php the_permalink() ?>">
									...
									<?php else: ?>
										<img width="600" height="450" src="<?php echo NOO_ASSETS_URI; ?>/images/thumb-news-bg.gif" class="attachment-noo-thumbnail-square wp-post-image" alt="<?php the_title(); ?>">
									<?php endif;?>
								</a>
						    </div>
...
if( !function_exists( 'noo_shortcode_blog' ) ) {
	function noo_shortcode_blog( $atts, $content = null ) {
		...
		$html[] = (!empty($title) ? '<h2 class="title">'.esc_html($title).(!empty($sub_title) ? '<p class="small sub-title">'.esc_html($sub_title).'</p>' : '').'</h2>' : '');
...
// [noo_portfolio]
// ============================
if( !function_exists( 'noo_shortcode_portfolio' ) ) {
	function noo_shortcode_portfolio ( $atts, $content = null ) {
		extract( shortcode_atts( array(
			'layout'            => 'masonry',
			'columns'           => '3',
			'grid_style'        => 'standard',
		...
		$class          = ( $class      != '' ) ? 'noo-portfolio ' . esc_attr( $class ) : 'noo-portfolio';
		$visibility     = ( $visibility != '' ) && ( $visibility != 'all' ) ? esc_attr( $visibility ) : '';
		$class         .= noo_visibility_class( $visibility );
		...
		$masonry_class .= ( $item_effect != '' ) ? ' masonry-style-' . $item_effect : '';
		
		$html = array();
		$portfolio_class  = '';
		$masonry_gutter   = '40';
		if( $layout == 'masonry' ) {
			$portfolio_class = 'masonry-portfolio';
			if( $grid_style == 'masonry' ) {
				//$portfolio_class .= ' no-gap';
				$masonry_gutter   = '0';
			}
		}
		$item_class='';
		...
		$html[] = '<div class="' . $portfolio_class . '">';
		...
		$html[] = '<div class="masonry-overlay"></div>';
		...
		$post_type = get_post_type($post_id);
		$post_format = noo_get_post_format($post_id, $post_type);
		$masonry_size = noo_get_post_meta($post_id, "_noo_portfolio_masonry_{$post_format}_size", 'regular');
		...
		?>
		<article id="post-<?php the_ID(); ?>" <?php post_class($item_class)?>>
			<div class="masory-item-wrap">
					<div class="content-featured">
						<?php noo_featured_portfolio(null,'',$lightbox,true); ?>
					</div>
				<div class="content-wrap">
					<header class="content-header">
	...
	add_shortcode( 'noo_portfolio', 'noo_shortcode_portfolio' );
}
