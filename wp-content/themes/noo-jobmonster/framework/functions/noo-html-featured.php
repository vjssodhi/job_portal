...
if (!function_exists('noo_get_featured_image')):
	function noo_get_featured_image($prefix = '_noo_wp_post', $post_id = null, $lightbox=false, $is_shortcode = false) {
		...
		if(!empty($thumb)) {
			if (!is_singular() || $lightbox  || $is_shortcode) {
				$html[] = '<a class="content-thumb'.( $lightbox ? ' noo-lightbox-item':'').'" '.($lightbox ? 'data-lightbox-gallery="portfolio-gallery"':'').'  href="' .($lightbox ? $post_thumbnail_src : esc_url(get_permalink())) . '" title="' . esc_attr(sprintf(__('"%s"', 'noo') , the_title_attribute('echo=0'))) . '">';
				$html[] = $thumb;
				$html[] = '</a>';
			} else {
...
if (!function_exists('noo_featured_image')):
	function noo_featured_image($prefix = '_noo_wp_post', $post_id = null,$lightbox = false,$is_shortcode = false) {
		echo noo_get_featured_image($prefix, $post_id, $lightbox, $is_shortcode);
	}
endif;
...
if (!function_exists('noo_featured_gallery')):
	function noo_featured_gallery( $prefix = '_noo_wp_post', $post_id = null,$lightbox = false ,$is_shortcode = false) {
		echo noo_get_featured_gallery( $prefix, $post_id,$lightbox, $is_shortcode);
	}
endif;
...
if (!function_exists('noo_get_featured_video')):
	function noo_get_featured_video($prefix = '_noo_wp_post', $post_id = null,$lightbox=false,  $is_shortcode = false) {
		...
		if ( $embed != '' ) {
			...
			<div id="noo-video-container<?php echo esc_attr($post_id); ?>" class="noo-video-container <?php echo esc_attr( $ratio_class ); ?>" <?php if($lightbox_preview):?> style="display: none;"<?php endif;?>>
...
if (!function_exists('noo_get_action_portfolio')):
	function noo_get_action_portfolio($post_id = null, $post_format = '',$lightbox = false, $is_shortcode = false) {
		$post_id = (null === $post_id) ? get_the_id() : $post_id;

		$prefix = '_noo_portfolio';
		$link = noo_get_post_meta($post_id, "{$prefix}_project_url", '#');
		$post_format = ('' === $post_format) ? noo_get_post_meta($post_id, "{$prefix}_media_type", 'image') : $post_format;
		$html  = array();
		switch ($post_format) {
			case 'image':
				$html[] = '<a class="noo-share noo-lightbox-item-trigger" href="#" >';
				$html[] = '<i class="fa fa-search-plus"></i>';
				$html[] = '</a>';
				return implode($html, "\n");
			case 'link':
				$html[] = '<a class="noo-share" href="'.esc_url($link).'" >';
...
if (!function_exists('noo_action_portfolio')):
	function noo_action_portfolio($post_id = null, $post_format = '',$lightbox=false,$is_shortcode = false) {
		echo noo_get_action_portfolio( $post_id, $post_format, $lightbox, $is_shortcode);
	}
endif;
...
function noo_featured_popup_portfolio($post_id=null,$post_format = ''){
	...
	if($post_format == 'image'){
		...
		$html = '<a class="noo-share noo-lightbox-item" href="'.esc_url(@$thumb[0]).'"><i class="fa fa-eye"></i></a>';
		echo $html;
		return;
