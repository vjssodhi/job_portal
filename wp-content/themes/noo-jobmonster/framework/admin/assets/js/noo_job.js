...
jQuery(document).ready(function($) {
	...
	$(".noo_job_custom_field_table").sortable({
			'items': 'tbody tr',
			'axis': 'y',
			 placeholder: "noo-job-state-highlight"
		});
		
		$('#add_noo_job_custom_field').click(function(){
			var table = $('.noo_job_custom_field_table'),
				n = 0,
				num = table.data('num');
			
			n = num + 1;
			var tmpl = noojobL10n.custom_field_tmpl.replace( /__i__|%i%/g, n );
			table.append(tmpl);
			table.data('num',n);
...
function toggle_disable_noo_job_custom_field(el) {
	$this = jQuery(el);
...
function delete_noo_job_custom_field(el){
